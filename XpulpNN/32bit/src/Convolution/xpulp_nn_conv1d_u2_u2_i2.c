                        ...
                        uint8_t flag_batch_norm)
{
  ...
  uint16_t dil = dilation - 1;

  int core_id = pi_core_id();
  uint8_t * pIm2ColBase = pIm2ColBuffer + (2 * core_id * PACK_INT2_SIZE(ch_in) * dim_kernel_y);
  int i_out_y, i_ker_y;
  int Log2Core;
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
      ...
      {
        for(i_ker_y=i_out_y * stride_y - padding_y_top; i_ker_y < i_out_y * stride_y - padding_y_top + dim_kernel_y * (1 + dil) - dil; i_ker_y += (1 + dil))
        {
            if((i_ker_y < 0) || (i_ker_y >= dim_in_y))
            {
              xpulp_nn_zero_mem_u2(pIm2Col, ch_in);
            }
            else
            {
              xpulp_nn_im2col_u2_to_u2((uint8_t*) pIn + i_ker_y * ch_in_r, pIm2Col, ch_in);
            }
      ...
      if(pIm2Col == (pIm2ColBase + ((PACK_INT2_SIZE(ch_in) * dim_kernel_y) << 1)))
      {
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      uint8_t out[4];
      uint16_t num_col_im2col = ch_in * dim_kernel_y;
      uint16_t num_col_im2col_w = PACK_INT2_SIZE(ch_in) * dim_kernel_y;
